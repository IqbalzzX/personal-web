...
if (isset($_GET['view'])) {
    $id = intval($_GET['view']);
    mysqli_query($db, "UPDATE tbl_artikel SET views = views + 1 WHERE id_artikel = $id");
...
if (isset($_POST['submit_komentar'])) {
    ...
    $sql = "INSERT INTO tbl_komentar (id_artikel, nama_pengirim, isi_komentar) VALUES ('$id_artikel', '$nama', '$komentar')";
    mysqli_query($db, $sql);
}
...
if ($search) {
    $sql = "SELECT * FROM tbl_artikel 
            WHERE nama_artikel LIKE '%$search%' 
            OR isi_artikel LIKE '%$search%' 
            ORDER BY id_artikel DESC 
            LIMIT $limit OFFSET $offset";

    $total_query = mysqli_query($db, "SELECT COUNT(*) AS total FROM tbl_artikel 
                                      WHERE nama_artikel LIKE '%$search%' 
                                      OR isi_artikel LIKE '%$search%'");
} else {
    $sql = "SELECT * FROM tbl_artikel 
...
<head>
    ...
    <title>Artikel | Iqbal Rizki Maulana</title>
    ...
    <style>
        ...
        /* CKEditor Figure Tables */
        .artikel-content figure.table {
            margin-bottom: 1rem;
            overflow-x: auto;
        }

        /* YouTube Embed via oembed */
...
<body>
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen }">
        ...
        <div class="flex flex-col flex-1 w-full">
            <header class="z-10 py-4 bg-white shadow-md dark:bg-gray-800">
                <div
                    ...
                    <!-- Search input -->
                    <form method="GET" class="flex justify-center flex-1 lg:mr-32">
                        <div class="relative w-full max-w-xl mr-6 focus-within:text-purple-500">
                            ...
                            <input
                                ...
                                type="text" name="search" placeholder="Search for projects" aria-label="Search" />
                        </div>
                    </form>
            ...
            <main class="h-full overflow-y-auto max-w-6xl mx-auto p-6 grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                <!-- Artikel -->
                <section class="md:col-span-2 bg-white dark:bg-gray-800 p-6 rounded shadow">
                    <h2 class="text-xl font-bold mb-4 dark:text-white">Artikel Terbaru</h2>
                    <div class="space-y-4">
                        <?php while ($data = mysqli_fetch_array($query)): ?>
                            <?php $id_artikel = $data['id_artikel']; ?>
                            <div class='border-b pb-4 mb-6'>
                                ...
                                <p class='text-sm text-gray-600'>
                                    Kategori: <span
                                        class='italic text-gray-800 dark:text-gray-400'><?= htmlspecialchars($data['kategori']); ?></span><br>
                                    Penulis: <strong><?= htmlspecialchars($data['penulis']); ?></strong> |
                                    Tanggal: <?= date('d M Y', strtotime($data['tanggal_posting'])); ?> |
                                    Dilihat: <?= $data['jumlah_view']; ?>x
                                </p>
                                <div class="artikel-content dark:text-white">
                                    <?= $data['isi_artikel']; ?>
                                </div>

                                <!-- Share -->
                                <?php
                                $judul_url = urlencode($data['nama_artikel']);
                                $link = base_url("views_artikel.php?id=$id_artikel");
                                ?>
                                <?php $id_artikel = $data['id_artikel']; ?>
                                <!-- Tombol utama -->
                                <button onclick="toggleShare('<?= $id_artikel ?>')"
                                    class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">
                                    <i class="fas fa-share-alt mr-1"></i> Bagikan
                                </button>

                                <!-- Share container -->
                                <div id="shareOptions-<?= $id_artikel ?>" class="mt-2 hidden flex flex-wrap gap-2">
                                    ...
                                    <a href="https://twitter.com/intent/tweet?url=<?= $link; ?>&text=<?= $judul_url; ?>"
                                        target="_blank"
                                        class="bg-sky-400 text-white px-3 py-1 rounded hover:bg-sky-500 text-sm flex items-center gap-1">
                                        <i class="fab fa-twitter"></i> <span class="hidden sm:inline">Twitter</span>
                                    </a>
                                    <a href="https://wa.me/?text=<?= urlencode($data['nama_artikel'] . ' - ' . $link); ?>"
                                        target="_blank"
     ...
     class="bg-pink-500 text-white px-3 py-1 rounded hover:bg-pink-600 text-sm flex items-center gap-1">
     <i class="fab fa-instagram"></i> <span class="hidden sm:inline">Instagram</span>
  </a> -->
                                    <button onclick="navigator.clipboard.writeText('<?= $link ?>'); alert('Link disalin!')"
                                        class="bg-gray-600 text-white px-3 py-1 rounded hover:bg-gray-700 text-sm flex items-center gap-1">
                                        <i class="fas fa-link"></i> <span class="hidden sm:inline">Copy Link</span>
                                </div>

                                <!-- Form Komentar -->
                                <form action='' method='post' class='mb-4 space-y-2'>
                                    ...
                                    <textarea name='komentar' placeholder='Tulis komentar...' required
                                        class='w-full border p-2 rounded dark:bg-gray-700 dark:text-white'></textarea>
                                    <button type='submit' name='submit_komentar'
                                        class='bg-blue-600 text-white px-4 py-1 rounded hover:bg-blue-700'>Kirim
                                        Komentar</button>
                                </form>

                                <!-- Komentar -->
                                <?php
                                $komen = mysqli_query($db, "SELECT * FROM tbl_komentar WHERE id_artikel=$id_artikel ORDER BY created_at DESC");
                                ?>
                                <div class='space-y-2'>
                                    <?php while ($k = mysqli_fetch_array($komen)): ?>
                                        <div class='border-t pt-2'>
                                            ...
                                            <p class='text-xs text-gray-500'><?= $k['created_at']; ?></p>
                                        </div>
                                    <?php endwhile; ?>
                                </div>
                            </div>
                        ...
                        <?php if (mysqli_num_rows($query) === 0): ?>
                            <p class="text-red-500">Tidak ada artikel ditemukan.</p>
                        <?php endif; ?>
                        ...
                        <div class="text-center mt-6">
                            <?php for ($i = 1; $i <= $total_pages; $i++): ?>
                                <a href="?page=<?= $i; ?>&search=<?= urlencode($search); ?>"
                                    class="dark:bg-gray-800 inline-block mx-1 px-3 py-1 border rounded <?= $i == $page ? 'bg-blue-600 text-white' : 'bg-white text-blue-600 border-blue-600' ?>">
                                    <?= $i; ?>
                                </a>
                            <?php endfor; ?>
                        </div>
                ...
                <!-- Sidebar -->
                <aside class="bg-white p-6 rounded shadow dark:bg-gray-800">
                    <h2 class="text-lg font-bold mb-4 dark:text-white">Daftar Artikel</h2>
                    <ul class="space-y-2 list-disc list-inside text-gray-700">
                        <?php
                        $daftar = mysqli_query($db, "SELECT id_artikel, nama_artikel FROM tbl_artikel ORDER BY id_artikel DESC");
                        while ($d = mysqli_fetch_array($daftar)) {
                            echo "<li><a href='views_artikel.php?id={$d['id_artikel']}' class='text-blue-600 hover:underline'>" . htmlspecialchars($d['nama_artikel']) . "</a></li>";
                        }
                        ?>
                    </ul>
                </aside>
    ...
    <script>
        function toggleShare(id) {
            const el = document.getElementById('shareOptions-' + id);
            el.classList.toggle('hidden');
        }
    </script>
    <script>
        // Convert oembed elements with YouTube URLs to iframes
        document.querySelectorAll("oembed[url]").forEach(el => {
            const url = el.getAttribute("url");
            const yt = url.match(/(?:https?:\/\/)?(?:www\.)?youtu(?:be\.com\/watch\?v=|\.be\/)([\w\-]+)/);
            if (yt) {
