    ...
    <div class="flex flex-col md:flex-row max-w-7xl mx-auto mt-8 px-4 space-y-4 md:space-y-0 md:space-x-6">
        ...
        <aside class="w-full md:w-1/4 bg-white rounded shadow p-4">
            ...
            <ul class="space-y-2 text-gray-700">
                ...
                <li><a href="data_berita.php" class="block font-semibold text-blue-800">Kelola Berita</a></li>
        ...
        <main class="w-full md:w-3/4 bg-white rounded shadow p-6">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 space-y-2 sm:space-y-0">
                ...
                <a href="add_berita.php" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">+ Tambah Berita</a>
            </div>
                ...
                <table class="min-w-full table-auto border-collapse">
                    ...
                    <tbody>
                        ...
                        while ($data = mysqli_fetch_array($query)) {
                            ...
                            echo "<td class='border px-4 py-2'>" . htmlspecialchars($data['judul']) . "</td>";
                            echo "<td class='border px-4 py-2'>" . htmlspecialchars($data['slug']) . "</td>";
                            echo "<td class='border px-4 py-2'>";
                            if ($data['gambar']) {
                                echo "<img src='" . htmlspecialchars($data['gambar']) . "' alt='Gambar Berita' class='w-20 h-auto'>";
                            } else {
                                echo "Tidak ada gambar";
                            }
                            echo "</td>";
                            echo "<td class='border px-4 py-2'>" . nl2br($data['isi']) . "</td>";
                            echo "<td class='border px-4 py-2'>" . htmlspecialchars($data['tag']) . "</td>";
                            echo "<td class='border px-4 py-2 text-center'>{$data['views']}</td>";
                            echo "<td class='border px-4 py-2'>" . htmlspecialchars($data['penulis']) . "</td>";
                            echo "<td class='border px-4 py-2 text-center'>{$data['status']}</td>";
                            echo "<td class='border px-4 py-2'>" . date('d-m-Y', strtotime($data['created_at'])) . "</td>";
                            echo "<td class='border px-4 py-2 text-center'>
                                    <a href='edit_berita.php?id={$data['id']}' class='text-blue-600 hover:underline'>Edit</a> |
                                    <a href='delete_berita.php?id={$data['id']}' onclick=\"return confirm('Yakin ingin menghapus berita ini?')\" class='text-red-600 hover:underline'>Hapus</a>
                                  </td>";
                            echo "</tr>";
                            $no++;
                        }
