...
$id = isset($_GET['id_artikel']) ? $_GET['id_artikel'] : (isset($_GET['id']) ? $_GET['id'] : null);

if (!$id) {
    echo "<script>alert('ID Artikel tidak ditemukan.'); window.location='data_artikel.php';</script>";
    exit;
}
...
<textarea id="editor" name="isi_artikel" rows="5" class="w-full p-2 border rounded focus:outline-none focus:ring
