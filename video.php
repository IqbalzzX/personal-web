...
<head>
    ...
    <title>Video | Iqbal Rizki Maulana</title>
...
<body>
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen }">
        ...
        <div class="flex flex-col flex-1 w-full">
            ...
            <main class="flex-1 w-full px-6 py-6 mx-auto">
                <?php
                // Ambil video dari database
                $result = $koneksi->query("SELECT * FROM tbl_video LIMIT 1");
                $data = $result->fetch_assoc();

                if ($data && !empty($data['url_video'])):
                    ?>
                    <div class="mb-6">
                        <h2 class="text-xl font-semibold text-gray-800 mb-4 dark:text-white">Video Terkini</h2>
                        <div class="aspect-w-16 aspect-h-9">
                            <iframe class="w-full h-64 rounded-lg shadow" src="<?= htmlspecialchars($data['url_video']) ?>"
                                allowfullscreen></iframe>
                        </div>
                    </div>
                    <div class="text-right">
                        <a href="<?= htmlspecialchars($data['url_video']) ?>" target="_blank"
                            class="inline-block bg-yellow-500 hover:bg-yellow-600 text-white px-5 py-2 rounded-lg shadow text-sm font-medium">
                            Tonton di YouTube
                        </a>
                    </div>
                <?php else: ?>
                    <p class="text-center text-gray-600">Belum ada video tersedia saat ini.</p>
                <?php endif; ?>
            </main>
        </div>
    </div>
