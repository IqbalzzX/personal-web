...
// Tambahkan views
$update = $db->prepare("UPDATE berita SET views = views + 1 WHERE id = ?");
$update->bind_param("i", $berita['id']);
$update->execute();
?>
<!DOCTYPE html>
...
<body>
    <div class="flex h-screen bg-gray-50 dark:bg-gray-900" :class="{ 'overflow-hidden': isSideMenuOpen }">
        ...
        <div class="flex flex-col flex-1 w-full">
            ...
            <main class="h-full overflow-y-auto w-full mx-auto p-6 mt-6">
                <section class="bg-white dark:bg-gray-800 p-6 rounded shadow">

                    <!-- Breadcrumb -->
                    <p class="text-xl text-gray-600 mb-2">
                        Berita >> <span class="text-blue-600"><?= htmlspecialchars($berita['judul']) ?></span>
                    </p>
                    ...
                    <div class="flex items-center text-sm text-gray-600 space-x-4 mb-6 dark:text-gray-400">
                        <span><i class="fa-regular fa-clock"></i>
                            <?= date("d M Y", strtotime($berita['created_at'])) ?></span>
                        <span><i class="fa-solid fa-user"></i> <?= htmlspecialchars($berita['penulis']) ?></span>
                        <span><i class="fa-solid fa-eye"></i> <?= $berita['views'] + 1 ?> Kali dibaca</span>
                    </div>

                    <!-- Gambar Berita -->
                    <?php if (!empty($berita['gambar'])): ?>
                        <img src="uploads/<?= $berita['gambar'] ?>" alt="Gambar Berita"
                            class="w-full h-auto rounded mb-6 shadow">
                    <?php endif; ?>

                    <!-- Isi Berita -->
                    <div class="artikel-content prose prose-lg text-justify leading-relaxed mb-6 dark:text-white">
                        <?= nl2br($berita['isi']) ?>
                    </div>
                    ...
                    <?php if (!empty($berita['tag'])): ?>
                        <div class="mt-6 pt-4 border-t border-gray-200">
                            <span class="inline-flex items-center px-3 py-1 bg-gray-200 text-gray-700 text-sm rounded-full dark:bg-gray-700 dark:text-gray-300">
